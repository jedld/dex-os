...
{
DWORD mem_size = 0 , i, i2, maximum_index,pg_stk_size = ((DWORD)(stackbase_end) - (DWORD)(stackbase));
stackbase[0]=0;

//compute maximum stack array, size in bytes divided by the size of a word which is 4 bytes

maximum_index = pg_stk_size/4 ;
   ...
   if (grub_meminfo[i].type == 1) 
       {
                    for (i2 = base; i2 < base_end; i2 += 0x1000)
                       {
                          if (((DWORD)stackbase + pg_stk_size) < i2 )
                             {
                                //Ensure the memory is within limits
        	                if ( (stackbase[0]+1) < maximum_index) {
        	                     stackbase[0]++;
...
/*
Initial the page table entries for the kernel, map 1-to-1 the first MB of memory and from 0x100000 to 0x300000
*/
void mem_init()
{
    DWORD i;
