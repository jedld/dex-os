...
name already exists in the destination, lzo_compress will return an error*/
int lzo_decompress(char *source, char *dest)
{
    ...
    fread(buf,header.cur_size,1,handle);
   
    int r = lzo1x_decompress(buf,header.cur_size,outbuf,&newlen,NULL);
    if (r == LZO_E_OK)
        printf("decompressed %lu bytes into %lu bytes\n",
            (unsigned long) size, (unsigned long) newlen);	
        else
        printf("decompression error\n");        
    printf("writing source file..\n");
    if (fwrite(outbuf,newlen,1,desthandle)==0)
    {
        /*Copy failed!!!*/
        fdelete(desthandle);
        fclose(handle);
...
/* Copies a file from one location to another, if a file with the same
name already exists in the destination, lzo_compress will return an error*/
int lzo_compress(char *source, char *dest)
{
    int i,newlen;
    char *buf,*outbuf;
    DWORD size;
    ...
    //allocate memory
    buf=(char*)malloc(size);
    outbuf=(char*)malloc(size+(size/2));
    printf("reading source file..\n");
    ...
    int r = lzo1x_1_compress(buf,size,outbuf,&newlen,workmem);
    if (r == LZO_E_OK)
        printf("compressed %lu bytes into %lu bytes\n",
            (unsigned long) size, (unsigned long) newlen);	
    else
        printf("compression error\n");
    header.cur_size =  newlen;       
    printf("writing source file..\n");
    fwrite(&header,sizeof(header),1,desthandle);
    if (fwrite(outbuf,newlen,1,desthandle)==0)
    {
        /*Copy failed!!!*/
        fdelete(desthandle);
  ...
  command_length = strlen(u);    
   ...
   if (strcmp(u,"loadmod")==0)
    ...
    if (strcmp(u,"lzip")==0) {
    		char *opt = strtok(0," ");
    		if (opt!=0) {
    			char *sourcefile = strtok(0," ");
    			if (sourcefile!=0) {
    				char *destfile = strtok(0," ");
    				if (destfile!=0) {
    				     if (strcmp(opt,"-c")==0) {	
	    				      if (lzo_compress(sourcefile,destfile)!=-1)
	    				        printf("compression done.\n");
	    				        else
	    				        printf("compression failed.\n");
	    				     } else 
	    			     if (strcmp(opt,"-x")==0) {
	    			     	 if (lzo_decompress(sourcefile,destfile)!=-1)
    				...
    				printf("missing destfile parameter.\n");
    			} else
    			  printf("missing sourcefile parameter.\n");
    		} else
...
void console_main()
  {
    ...
    if (console_first == 0) script_load("/boot/startup.sh");
